...
class NetworkLstm(tnn.Module):
    ...
    def __init__(self):
        ...
        inputSize = 50
        hiddenSize = 100
        self.Lstm = torch.nn.LSTM(input_size=inputSize, hidden_size=hiddenSize, batch_first=True)
...
class NetworkCnn(tnn.Module):
    ...
    def __init__(self):
        ...
        Create and initialise weights and biases for the layers.
        """
        self.conv = nn.Conv1d(in_channels=50, out_channels=50, kernel_size=8, padding=5)
    ...
    def forward(self, input, length):
        ...
        x3 = self.max_pool(x2,4)
        x4 = self.conv(x3)
        x5 = torch.nn.functional.relu(x4)
        x6 = self.max_pool(x5,4)
        x7 = self.conv(x6)
        x8 = torch.nn.functional.relu(x7)
        x9 = self.max_pool(x8,2).values
        x10 = self.fc(x9)
        x11 = torch.squeenze(x10)
        return x11
...
def measures(outputs, labels):
    ...
    a1 = torch.round(a).int()
    i = outputs
    j = labels
    positive = torch.sum(i,0)
    negative = j.size()[0] - positive

    tp = torch.sum(i&j,0)
    tn = torch.sum((1-i) & (1-j),0)
    fp = positive - tp
    fn = i.size()[0] - tp - tn -fp
