...
class rnn(torch.nn.Module):
    ...
    def rnnCell(self, input, hidden):
        ...
        """
        rnncell = self.ih(input) + self.hh(hidden)
        return torch.tanh(rnncell)
...
def lstm(input, hiddenSize):
    ...
    """
    a = input.shape[2]
    lstm = torch.nn.LSTM(input_size=a, hidden_size=hiddenSize, batch_first=True)
    return lstm(input)
...
def conv(input, weight):
    ...
    """
    return torch.nn.functinal.conv1d(input,weight)
